...
class DailyMaintenanceTests(unittest.TestCase):
    ...
    def test_report_is_deterministic_bounded_and_idempotent(self) -> None:
        ...
        MODULE.atomic_write(running_state["running"], running_day.isoformat())
        ...
        stale_day = dt.datetime(2026, 8, 2, 9, 15, tzinfo=dt.timezone.utc)
        stale_state = MODULE.state_paths(self.paths, "2026-08-02")
        MODULE.atomic_write(stale_state["running"], (stale_day - dt.timedelta(minutes=66)).isoformat())
        self.assertIn("limite maximale", MODULE.report(paths=self.paths, now=stale_day))
        MODULE.atomic_write(stale_state["result"], "late-after-timeout")
        self.assertEqual(MODULE.report(paths=self.paths, now=stale_day), "")
        self.assertFalse(stale_state["result"].exists())

        early_missing = dt.datetime(2026, 8, 3, 9, 0, tzinfo=dt.timezone.utc)
        missing_day = dt.datetime(2026, 8, 3, 9, 15, tzinfo=dt.timezone.utc)
        missing_state = MODULE.state_paths(self.paths, "2026-08-03")
        self.assertEqual(MODULE.report(paths=self.paths, now=early_missing), "")
